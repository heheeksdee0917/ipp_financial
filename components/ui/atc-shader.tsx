...
export default function ShaderDemo_ATC() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      // Fix 1: Render at half resolution
      const dpr = 0.5
      // Fix 2: Cap max canvas size
      const w = Math.min(Math.floor((canvas.clientWidth || window.innerWidth) * dpr), 960)
      const h = Math.min(Math.floor((canvas.clientHeight || window.innerHeight) * dpr), 540)
    ...
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      <canvas
        ...
        style={{
          ...
          display: "block",
          background: "#000",
          // Fix 3: Let CSS scale up the low-res canvas smoothly
          imageRendering: "auto",
        }}
      />
      <pre
        ref={preRef}
        style={{ position: "absolute", top: 8, left: 8, color: "#0f0", whiteSpace: "pre-wrap" }}
      />
    </div>
  )
}